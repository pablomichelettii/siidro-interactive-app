...
app.get("/qr", async (req, reply) => {
  ...
  const buf = await QRCode.toBuffer(String(data), { width: 640, margin: 1, color: { dark: "#0a0a0a", light: "#ffffff" } });
...
io.on("connection", (socket) => {
  ...
  if (role === "main") {
    ...
    socket.on("director:goto", (d) => session.goto(d && d.id));
    socket.on("director:reset", () => session.reset());
    // chat: niente arriva sul proiettore se non lo manda il regista
    socket.emit("director:chatQueue", session.coda);   // chi rientra ritrova la coda
    socket.on("director:chatApprove", (d) => session.chatApprove(d && d.id));
    socket.on("director:chatReject", (d) => session.chatReject(d && d.id));
    socket.on("director:chatMute", (d) => session.chatMute(d && d.cid));
    socket.on("director:chatClear", () => session.chatClear());
    ...
    socket.on("device:chat", (d, ack) => {
      const r = session.chat(socket.data.cid, d && d.testo);
...
console.log(`\n  SIIDRO interactive app — in ascolto su :${PORT}`);
